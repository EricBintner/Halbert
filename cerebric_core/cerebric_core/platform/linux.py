...
- journald for log collection (via cerebric-linux adapter)
- hwmon for sensors (via cerebric-linux adapter)
- systemd for service management (via cerebric-linux adapter)
...
linux_adapter_path = os.path.join(os.path.dirname(__file__), '../../../cerebric-linux')
...
class LinuxPlatformBridge(PlatformBridge):
    ...
    def read_sensors(self) -> List[Dict[str, Any]]:
        ...
        """
        if self._hwmon:
            # Use hwmon adapter (returns normalized cerebric format)
