...
export function ConfigEditor({ filePath, onClose }: ConfigEditorProps) {
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent('cerebric:set-config-context', {
      detail: { filePath, getContent }
    }));
    
    // Cleanup: clear config context when unmounting
    return () => {
      window.dispatchEvent(new CustomEvent('cerebric:clear-config-context'));
    };
  }, [filePath]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('cerebric:apply-edit', handleApplyEdit as EventListener);
    return () => {
      window.removeEventListener('cerebric:apply-edit', handleApplyEdit as EventListener);
    };
  }, [content]);
