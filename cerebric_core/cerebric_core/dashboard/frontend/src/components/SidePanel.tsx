 ...
 * - Listens for 'cerebric:run-command' events to auto-run commands
...
export function SidePanel() {
  ...
  const [aiName, setAiName] = useState('Cerebric')
  ...
  useEffect(() => {
    ...
    api.getPersonaNames()
      .then(data => {
        const name = data.ai_name || 'Cerebric'
        setAiName(name)
        initMessages(name)
      })
      .catch(() => {
        // Fallback to default
        initMessages('Cerebric')
      })
    loadMentionables()
    loadConversations()
  ...
  */
  ...
  useEffect(() => {
    ...
    window.addEventListener('cerebric:run-command', handleRunCommand as EventListener)
    return () => {
      window.removeEventListener('cerebric:run-command', handleRunCommand as EventListener)
    }
  }, [])
  ...
  useEffect(() => {
    ...
    window.addEventListener('cerebric:send-to-chat', handleSendToChat as EventListener)
    return () => {
      window.removeEventListener('cerebric:send-to-chat', handleSendToChat as EventListener)
    }
  }, [])
  ...
  useEffect(() => {
    ...
    window.addEventListener('cerebric:open-chat', handleOpenChat as unknown as EventListener)
    return () => {
      window.removeEventListener('cerebric:open-chat', handleOpenChat as unknown as EventListener)
    }
  }, [])
  ...
  useEffect(() => {
    ...
    window.addEventListener('cerebric:set-config-context', handleSetConfigContext as EventListener)
    window.addEventListener('cerebric:clear-config-context', handleClearConfigContext as EventListener)
    return () => {
      window.removeEventListener('cerebric:set-config-context', handleSetConfigContext as EventListener)
      window.removeEventListener('cerebric:clear-config-context', handleClearConfigContext as EventListener)
    }
  }, [])
                          ...
                          {message.editBlocks.map((block, idx) => (
                            <div key={idx} className="border rounded p-2 bg-background/50">
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  window.dispatchEvent(new CustomEvent('cerebric:apply-edit', {
