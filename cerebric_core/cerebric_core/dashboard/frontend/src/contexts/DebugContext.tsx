...
export function DebugProvider({ children }: { children: ReactNode }) {
  const [isDebugMode, setIsDebugMode] = useState(() => {
    ...
    return localStorage.getItem('cerebric_debug_mode') === 'true'
  })
  ...
  const setDebugMode = useCallback((enabled: boolean) => {
    ...
    localStorage.setItem('cerebric_debug_mode', enabled.toString())
    if (enabled) {
      console.log('%c[Cerebric Debug Mode Enabled]', 'color: #22c55e; font-weight: bold')
    }
  }, [])
