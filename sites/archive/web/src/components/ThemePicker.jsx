...
export function ThemePicker({ defaultTheme = 'olivetti-1968' }) {
    ...
    <aside aria-label="Developer theme picker" className="fixed bottom-4 right-4 z-50 font-mono select-none">
      ...
      {!isOpen && (
        <button
          ...
          className="px-3.5 py-2 bg-[var(--color-ink)] text-white text-xs font-bold uppercase tracking-wider border-2 border-[var(--color-ink)] shadow-[4px_4px_0px_0px_rgba(211,78,36,1)] hover:bg-[var(--color-accent)] transition-all flex items-center space-x-2"
        >
        ...
        <div className="w-80 sm:w-96 bg-[var(--color-surface)] border-2 border-[var(--color-ink)] shadow-[8px_8px_0px_0px_rgba(18,20,23,1)] p-5 space-y-4 text-xs text-[var(--color-ink)]">
          ...
          <div className="flex justify-between items-baseline border-b-2 border-[var(--color-ink)] pb-2.5">
            <div className="space-y-0.5">
              <div className="font-display font-extrabold text-sm tracking-tight text-[var(--color-ink)]">
                MID-CENTURY COLOR MATRIX
              </div>
