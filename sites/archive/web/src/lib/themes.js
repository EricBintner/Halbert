 ...
 * 1. Olivetti 1968 (Vermilion on Warm Archival Paper)
 * 2. Bauhaus Signal Amber (Signal Amber & Drafting Bone)
...
export function applyTheme(themeId) {
  ...
  // Update html and body inline backgrounds for instant sync
  ...
  try {
    localStorage.setItem('halbert_dev_theme', theme.id);
  } catch (e) {
...
/**
 * Retrieves the saved theme or falls back to default
 */
export function getSavedTheme(defaultThemeId = 'olivetti-1968') {
  try {
    const saved = localStorage.getItem('halbert_dev_theme');
