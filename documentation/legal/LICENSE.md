# License

Cerebric is licensed under the **GNU General Public License v3.0** (GPL-3.0).

## Summary

**You are free to:**

- **Use** - Run the software for any purpose
- **Study** - Examine and modify the source code
- **Share** - Copy and distribute the software
- **Improve** - Modify and distribute your modifications

**Under the following conditions:**

- **Disclose source** - Source code must be available when distributing
- **Same license** - Modifications must use GPL-3.0
- **State changes** - Document changes made to the code
- **License notice** - Include the license in distributions

## Full License

The complete GPL-3.0 license text is available at:

- [GNU GPL-3.0 Official Text](https://www.gnu.org/licenses/gpl-3.0.en.html)
- [SPDX License](https://spdx.org/licenses/GPL-3.0-only.html)

## For Contributors

By contributing to Cerebric, you agree that your contributions will be licensed under GPL-3.0.

## Why GPL-3.0?

GPL-3.0 ensures that:

1. The software remains free and open source
2. Improvements benefit the community
3. Users have the right to study and modify the code
4. Commercial use requires sharing modifications

This aligns with the project's goal of building a community around Linux system management.
