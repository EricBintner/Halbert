...
**Scope:** Halbert Software (Core CLI, Desktop Application, Background Daemons) and the Halbert Project Website (`halbert.computer` / marketing web).
...
- Environment variables (`$PATH`, and Halbert's own override variables).

**All of this data remains strictly on your local machine.** It is stored in standard local directories: on Linux, per the XDG Base Directory Specification:
- Configuration: `~/.config/halbert/`
- Data & Vector Indices: `~/.local/share/halbert/`
- State & Local Logs: `~/.local/state/halbert/`

On macOS: `~/Library/Application Support/Halbert/` and `~/Library/Caches/Halbert/`.
...
If you **explicitly choose** to configure external cloud API keys (e.g., Anthropic Claude, OpenAI, or OpenRouter):
...
- **Storage & Retention**: We do not sell, rent, trade, or share your email address with third parties or data brokers. You may unsubscribe or request permanent deletion at any time by contacting `hello@halbert.computer`.
...
For the paid tier (Halbert Pro for macOS):
...
- License verification is offline (Ed25519); the application never phones home during license checks.
...
To exercise any of these rights, email: `hello@halbert.computer`.
