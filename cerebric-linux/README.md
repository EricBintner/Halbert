# Cerebric Linux Platform Adapter
...
This package provides Linux-specific adapters that integrate with the Cerebric platform abstraction layer. It wraps existing Linux tools and libraries to provide a consistent interface.
...
- Normalizes readings to Cerebric format
...
These adapters wrap existing `cerebric_core` functionality:

**JournaldAdapter** wraps:
- `cerebric_core.ingestion.journald.follow_journal()`
- `cerebric_core.ingestion.journald._normalize()`

**HwmonAdapter** wraps:
- `cerebric_core.ingestion.hwmon.collect_temp()`
...
This package is automatically included when installing Cerebric on Linux:

```bash
pip install cerebric-linux
```
...
Linux Adapter (platform-specific)
    ↓
Existing cerebric_core code (reused)
```
...
- ✅ cerebric-linux/ (this directory)
- ✅ All adapter code
- ✅ Linux-specific implementations

**Private** (gitignored):
- cerebric-mac/ (macOS equivalent)

---

## Related

- **Platform Abstraction**: `cerebric_core/platform/`
- **Tests**: `tests/platform/`
- **Mac Adapter**: `cerebric-mac/adapters/` (gitignored)

---

**Version**: 0.1.0  
**License**: Same as Cerebric  
**Maintainer**: Cerebric team
