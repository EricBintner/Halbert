...
try:
    ...
    from halbert_core.halbert_core.config.watcher import ConfigWatcher
    from halbert_core.halbert_core.config.drift import diff_snapshots
    from halbert_core.halbert_core.obs.dashboard import build_all as build_dashboard_all
    from halbert_core.halbert_core.index.chroma_index import Index as HalbertIndex
...
def cmd_ingest_journald(args):
    ...
    if run_journald is None:
        print('halbert_core not available or dependencies missing (pyyaml).')
        return
...
def cmd_watch_configs(args):
    ...
    if ConfigWatcher is None:
        print('halbert_core not available or dependencies missing (watchdog/pyyaml).')
        return
...
def cmd_policy_eval(args):
    ...
    try:
        ...
        return
    from halbert_core.halbert_core.policy.engine import decide as _decide
...
def cmd_config_validate(args):
    ...
    try:
        ...
        if wizard.validate_config():
            ...
            print("   Run 'halbert config-wizard' to fix")
...
def cmd_mlx_prepare_training_data(args):
    ...
    try:
        from halbert_core.halbert_core.model.training_data import (
...
def cmd_performance_alerts(args):
    ...
    try:
        from halbert_core.halbert_core.model.performance_monitor import AlertSeverity
...
def main():
    ...
    p_show = sub.add_parser('show', help='Show a project file (relative to Halbert/)')
    ...
    p_dashboard = sub.add_parser('dashboard', help='Start Halbert web dashboard (Phase 3 M5)')
    ...
    def _cmd_ask(args):
        try:
            ...
            from halbert_core.halbert_core.rag.llm import OllamaLLM, LLMConfig
            
            question = ' '.join(args.question) if args.question else ''
            if not question:
                print('Usage: halbert ask "your question here"')
                return
    ...
    def _cmd_rag_add(args):
        try:
            from halbert_core.halbert_core.rag.ingestion import RAGIngestionEngine
            ...
            if result.success:
                ...
                print(f'\n💡 Run "halbert rag-merge" to update the corpus index.')
            else:
    ...
    def _cmd_rag_sources(args):
        try:
            from halbert_core.halbert_core.rag.ingestion import SourceRegistry
    ...
    def _cmd_rag_check(args):
        try:
            from halbert_core.halbert_core.rag.ingestion import (
            ...
            if is_valid and (source_info or True):  # Can be added
                print('✅ Ready to add: halbert rag-add ' + args.url)
            else:
    ...
    def _cmd_rag_bulk(args):
        try:
            ...
            if success > 0:
                print(f'\n💡 Run "halbert rag-merge" to update the corpus.')
