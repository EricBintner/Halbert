...
> **STATUS UPDATE (2026-08-31 evening):** most of the backlog below was executed by a parallel session on this branch (O1–O6, O9, G1–G4, P3) and **v2 founder tuning** was layered on the mark engine (branch `feat/voice-mode-mark-v2`): medium 6-tine default, plucked-string speaking (PLUCK_SPRING + onset impulses), thinking "snake-ate-a-ball" traveling bulges. Combined tree verified: 68/68 design-system, 556/556 dashboard frontend, 80 voice/audio backend tests. **Still open:** O7–O8 (`VoiceMode.tsx` + `/voice` route & shell integration — the headline UI), P1 (standby tier controller), P2 (display-power daemon, greenfield), P4 (Tauri window / voice-hud decision), P5 (hardware validation).
