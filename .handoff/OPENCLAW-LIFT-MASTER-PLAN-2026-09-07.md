...
| Packet | Branch | Touches (primary) | Session |
|---|---|---|---|
| 01-A | `feat/lift-01a-promotion` | `continuity/promotion.py` (new), `threads.py` (recall site), `tools/recall_memory.py` | background executor (sonnet/high) |
| 02-A/B | `feat/lift-02ab-lattice` | `persona/policy.py`, `persona/admission.py`, `persona/claims.py` (all new) + tests | background executor (sonnet/high) |
| 03-A | `feat/lift-03a-scheduler` | `scheduler/catchup.py`, `restart_budget.py`, `run_receipts.py`, monitor-hash (all new) | background executor (sonnet/high) |
| 04-A1/B/C1 | `feat/lift-04-voice-ingress` | `routes/agent.py` (request model), `app.py` (_relay_voice_turn), `state_machine.py` (process + responding finalize), `tools/executor.py` (digest), `modality_wiring.py`, frontend `pcmCapture.ts`/`VoiceMode.tsx` | background executor (opus/high) |
| 05-A/C2 | `feat/lift-05a-redaction` | `ingestion/redaction_registry.py` (new), `config/queries.py` (one line), `mcp/response.py`, `mcp/server.py` or `__init__.py` | background executor (sonnet/high) |
| 06-A/B | `feat/lift-06-execute-code` | `tools/script_stubs.py`, `tools/script_rpc_host.py`, `tools/execute_code.py` (new), `tools/executor.py` (register) | background executor (opus/xhigh) |
| 07-A | `feat/lift-07a-algebra` | `agents/turn_activity.py`, `agents/steering.py` (new) | background executor (sonnet/med) |
| 08 | `feat/lift-08-store-hardening` | `agents/conversation_sqlite.py`, `continuity/state_store.py`, `scheduler/engine.py` (JSON atomic) | background executor (sonnet/high) |
| 09-A | `feat/lift-09a-harness` | `continuity/eval_corpus.py`, `eval_recall.py` (extend), `eval_consolidation.py` (new), `halbert_core/evals/` | background executor (opus/high) |
| 09-B | `feat/lift-09b-probes` | `halbert_core/probes/` (new) | background executor (sonnet/high) |
| D-7 | `feat/lift-d7-utility-slot` | model picker/config modules (verified at start) | background executor (sonnet/high) |
| EN-1 | `feat/engine-en1-provenance` (Haloysius) | `src/haloysius/memory_v2/types.py`, `store.py` | background executor in Haloysius worktree (sonnet/high) |
| D-1 | — (docs only) | `.handoff/DESIGN-SESSION-TREE-AND-COMPACTION-2026-09-07.md` | fable design pass |
| D-2 | — (docs only) | `.handoff/DESIGN-SKILLS-SYSTEM-2026-09-07.md` | fable design pass |
