...
- [ ] Variant-gate SourcePrep wiring out of `home`/`home-light`: retrieval backend not instantiated, no config-watcher reindex callback, HA-config bridge surface retired (handoff S2, W7–W12).
- [ ] Replace the model picker with a "Compute Peer" setting (hostname:port + "Test Connection") for `home`/`home-light`; register `PeerProvider` in the model stack (handoff S3, W14–W16).
- [ ] Drop the 1B model tier from `recommend_budget()`/`get_installation_commands()`; `SBC_LOW_POWER` = offload only (subject to decision D2); add the wizard compute-peer prompt (handoff S4, W17–W19).
