...
- Unified Logging (log stream) for logs (via halbert-mac adapter)
- IOKit/powermetrics for sensors (via halbert-mac adapter)
- launchd for service management (via halbert-mac adapter)
...
# Add halbert-mac to path
mac_adapter_path = os.path.join(os.path.dirname(__file__), '../../../halbert-mac')
...
class MacPlatformBridge(PlatformBridge):
    ...
    def read_sensors(self) -> List[Dict[str, Any]]:
        ...
        """
        if self._iokit:
            # Use IOKit adapter (returns normalized halbert format)
