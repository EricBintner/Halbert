 ...
 *
 * See plan-b-contracts.md section 11.
 */
...
const TOKEN_CLASS: Record<StatusLightState, string> = {
...
function labelText(props: StatusLightProps): string {
  ...
  if (props.state === 'done_unseen') {
    return `exit ${props.exitCode ?? 0}`;
  }
  if (props.state === 'error') {
    return `exit ${props.exitCode ?? 1}`;
  }
...
export function StatusLight({ state, elapsedSeconds, exitCode, label, size = 'sm' }: StatusLightProps): ReactElement {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 ${TOKEN_CLASS[state]}`}
      style={{ transition: 'color var(--duration-shutter) var(--ease-shutter)' }}
      role="img"
