...
logger = logging.getLogger('halbert.dashboard.routes.conversations')

router = APIRouter() if FASTAPI_AVAILABLE else None

# Storage directory
CONVERSATIONS_DIR = Path.home() / '.config' / 'halbert' / 'conversations'
PREFERENCES_PATH = Path.home() / '.config' / 'halbert' / 'preferences.yml'


def get_ai_name() -> str:
    """Get the AI name from preferences with fallback chain.
    
    Priority: ai_name from preferences > hostname > "Halbert"
    """
    import socket
    import yaml
