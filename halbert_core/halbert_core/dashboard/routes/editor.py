...
class BackupRestoreRequest(BaseModel):
    #: SEC-3 (F28): this was joined straight into ``backup_dir / f"{backup_id}.bak"``,
    #: so ``../../../etc/passwd`` read outside the backup directory — and the
    #: content it read was then written to the caller's target path.
    #:
    #: A backup id is generated by ``create_backup`` as exactly
    #: ``timestamp.strftime("%Y%m%d-%H%M%S")``, so pinning that shape costs
    #: nothing and leaves no room to express a path at all. Validated on the
    #: model rather than in the handler: a second restore route added later
    #: inherits the constraint instead of having to remember it.
    backup_id: str = Field(..., pattern=r"^\d{8}-\d{6}$")
