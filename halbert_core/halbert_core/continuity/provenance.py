...
ERASURE_LIMITS = (
    ...
    "which are redacted separately; the promotion signals recorded in this "
    "process (erased with the request; signals a PREVIOUS process recorded "
    "are keyed by claim and outlive their run id); the event ledger "
    "(timeline.db), whose "
    "sensor, occupancy and state rows this does not touch -- erasing what it "
    "holds about one person is a separate operation, by subject rather than "
    "by request; "
