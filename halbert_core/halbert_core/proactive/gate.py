...
class ProactiveGate:
    ...
    def should_notify(self, event: ProactiveEvent) -> Tuple[bool, str]:
        ...
        """
        # 1. Check proactivity dial
        dial = self.config.proactivity
        ...
        if self.guardrails.safe_mode_active and event.severity != "critical":
                ...
                if finding.status == FindingStatus.SNOOZED.value:
                    ...
                    if finding.snoozed_until:
                        try:
                            ...
                            pass  # Invalid date format, allow notification
