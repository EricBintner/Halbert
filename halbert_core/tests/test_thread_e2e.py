...
class ScriptedLLM:
    ...
    def __init__(self):
        ...
        self.script = {}
        self.replies = {}
        self.chat_prompts = []    # (turn, flattened prompt)
        self.stream_prompts = []  # (turn, flattened prompt)
        self.chat_arrays = []     # (turn, messages[])
        self.stream_arrays = []   # (turn, messages[])

    async def chat(self, messages, tools=None, **kwargs):
        self.chat_arrays.append((self.turn, messages))
    ...
    async def stream(self, messages, **kwargs):
        self.stream_arrays.append((self.turn, messages))
        prompt = "\n".join(m["content"] for m in messages)
    ...
    def planning_array(self, turn):
        return next(m for t, m in self.chat_arrays if t == turn)

    def responding_array(self, turn):
        return next(m for t, m in self.stream_arrays if t == turn)
...
async def test_second_message_sees_the_first(agent, llm, tm, store):
    ...
    # PLANNING on turn 2 carries the continuity hint on the LAST message,
    # next to the question (merge D1): the head of the array is what a small
    # context window drops, so the hint no longer sits in the prose.
    planning_2 = llm.planning_array(2)
    assert planning_2[0]["role"] == "system"
    assert "## Current Task" in planning_2[0]["content"]
    assert "<continuity>" not in planning_2[0]["content"]
    assert "<continuity>" in planning_2[-1]["content"]
    assert "family photos" in planning_2[-1]["content"]
    assert planning_2[-1]["content"].endswith(MSG_2)

    # RESPONDING on turn 2 sees the raw first exchange as real messages
    # rather than a "## Earlier in this conversation" prose block: one
    # budget, one place (merge E-3).
    responding_2 = llm.responding_array(2)
    assert [m["role"] for m in responding_2] == ["system", "user", "assistant", "user"]
    assert responding_2[1]["content"] == MSG_1
    assert responding_2[2]["content"] == REPLY_1
    assert "/etc/samba/smb.conf" in responding_2[2]["content"]
    assert "Earlier in this conversation" not in llm.responding_prompt(2)

    # turn 1 did not see anything (nothing to see)
    assert [m["role"] for m in llm.responding_array(1)] == ["system", "user"]
...
async def test_past_reference_pulls_in_closed_thread_without_a_tool_call(
    ...
    planning_4 = llm.planning_array(4)
    assert "samba" in "\n".join(m["content"] for m in planning_4).lower()
    # The hint (with its "Pulled in:" line) is on the question, not in the
    # prose head (merge D1).
    assert "<continuity>" not in planning_4[0]["content"]
    assert "<continuity>" in planning_4[-1]["content"]
    assert "Pulled in:" in planning_4[-1]["content"], planning_4[-1]["content"]
    assert planning_4[-1]["content"].endswith(MSG_4)
