...
class TestGuestToolAllowlist:
    ...
    def test_the_home_and_the_view_are_on_the_list(self):
        for tool in ("ha_get_entity_state", "ha_call_service", "frigate_list_cameras",
                     ...
                     "recall_memory", HANDBACK_TOOL_NAME):
            assert tool in GUEST_ALLOWED_TOOLS, tool

    def test_filter_keeps_order_and_drops_the_rest(self):
        tools = ["run_command", "ha_get_entity_state", "read_file", "recall_memory"]
        assert filter_tools_for_guest(tools) == ["ha_get_entity_state", "recall_memory"]
    ...
    def test_every_allowlisted_tool_is_a_real_agent_tool(self):
        ...
        missing = (GUEST_ALLOWED_TOOLS - {HANDBACK_TOOL_NAME}) - registered
...
def _stub_executor(calls):
    ...
    async def _recall(args):
        calls.append(("recall_memory", args))
        return "remembered"

    executor.register("run_command", _run, {"name": "run_command", "parameters": {}})
    executor.register("recall_memory", _recall, {"name": "recall_memory", "parameters": {}})
    return executor
...
class TestExecuteWhileAGuestFronts:
    ...
    async def test_an_allowed_tool_still_runs(self):
        ...
        executor = _stub_executor(calls)
        _front()
        result = await executor.execute("recall_memory", {"q": "garage"}, session_id="s1")
        assert result.success is True
        assert result.result == "remembered"
        assert ("recall_memory", {"q": "garage"}) in calls
